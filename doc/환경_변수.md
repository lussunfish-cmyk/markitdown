...
| 환경 변수 | 기본값 | 설명 |
|---------|--------|------|
| `CHUNK_SIZE` | `800` | 청크 크기 (문자 단위) |
| `CHUNK_OVERLAP` | `200` | 청크 간 겹침 크기 (문자 단위) |
...
| 환경 변수 | 기본값 | 설명 |
|---------|--------|------|
| `RETRIEVER_HYBRID_ALPHA` | `0.8` | 하이브리드 검색 가중치 (0~1) |
| `RETRIEVER_RRF_K` | `60` | RRF 파라미터 |
| `RETRIEVER_DEFAULT_TYPE` | `advanced` | 기본 검색 타입 |
| `RETRIEVER_RERANKER_MODEL` | `BAAI/bge-reranker-v2-m3` | 리랭커 모델명 (Cross-Encoder) |
...
- **HYBRID_ALPHA**: 
  - `1.0`: 벡터 검색만 사용
  - `0.5`: 벡터 검색 50%, 키워드 검색 50% (config.py 기본값)
  - `0.0`: 키워드 검색만 사용
...
# 리랭커 비활성화 (속도 향상)
export RETRIEVER_USE_RERANKER=false
...
| `RAG_MAX_TOKENS` | `2048` | 생성할 최대 토큰 수 |
| `RAG_MAX_CONTEXT_LENGTH` | `8192` | 최대 컨텍스트 길이 (문자) |
| `RAG_ENABLE_QUERY_REWRITING` | `true` | 쿼리 재작성 활성화 여부 |
