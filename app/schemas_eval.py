...
class TestsetGenerateRequest(BaseModel):
    ...
    input_dir: str = Field("./output", description="테스트셋 생성에 사용할 문서가 있는 디렉토리 경로")
    output_file: str = Field("./scripts/testset.csv", description="생성된 테스트셋을 저장할 CSV 파일 경로")
...
class EvaluationRequest(BaseModel):
    testset_file: str = "./scripts/testset.csv"
    top_k: int = 5
