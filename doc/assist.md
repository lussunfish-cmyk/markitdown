...
main.py: /convert API가 FileResponse로 파일을 직접 다운로드 제공
output 폴더에도 저장됨 (기존 기능 유지)
동작 방식:
...
다운로드: converted.md
저장: ./output/document.md
이제 단일 파일은 바로 다운로드되고, 폴더 배치는 /convert-folder로 처리됩니다! ✅
...
로깅 시스템 추가: 변환 진행 상황을 터미널에 실시간 출력
변환 시간 측정: 각 파일별 변환 소요 시간 기록
JSON 결과 다운로드: /convert-folder API가 결과를 JSON 파일로 다운로드
터미널 로그 예시
Edited
main.py+107-29
JSON 결과 다운로드
...
이제 터미널에서 실시간으로 변환 진행 상황을 확인하고, 완료 후 JSON 결과를 다운로드할 수 있습니다! ✅
...
Dockerfile: pandoc 설치 제거
이미지 빌드 시간이 더 빨라집니다 (Pandoc 제거로 약 20-30초 단축)
현재는 LibreOffice만 사용하여 .doc → .docx 변환하고 있으므로 불필요한 패키지가 제거되었습니다. ✅

app/main.py → converter.py
Dockerfile: app.main:app → app.converter:app
재시작:

Edited
Dockerfile+1-1
파일명이 기능을 명확하게 나타내도록 변경되었습니다! ✅
